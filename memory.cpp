...
uintptr_t getExportedFunctionAddress(HANDLE hProcess, uintptr_t moduleBase, const wchar_t* moduleName, const char* functionName) {
    ...
    // Guard against forwarded exports: if GetProcAddress resolved the name to a
    // function that lives in a *different* DLL (e.g. USER32 forwarding to win32u,
    // or KERNEL32 forwarding to kernelbase), then the local address is not inside
    // hLocalModule and the offset below would be meaningless. Applying that bogus
    // offset to the remote module base would make us hook a random address.
    // For DLLs, HMODULE == the module's base address, so an address that belongs to
    // this module shares its allocation base. If it doesn't, refuse rather than
    // corrupt an unrelated location in the target process.
    MEMORY_BASIC_INFORMATION mbi{};
    if (VirtualQuery(reinterpret_cast<void*>(localProcAddress), &mbi, sizeof(mbi)) == 0 ||
        mbi.AllocationBase != reinterpret_cast<void*>(hLocalModule)) {
        std::cout << "Error: Export appears to be forwarded to another module; refusing to compute a bogus offset." << std::endl;
        FreeLibrary(hLocalModule);
        return 0;
    }
...
bool readMemory(HANDLE hProcess, uintptr_t address, void* buffer, SIZE_T size) {
    SIZE_T read;
    return ReadProcessMemory(hProcess, reinterpret_cast<void*>(address), buffer, size, &read) && read == size;
}
...
bool protectMemory(HANDLE hProcess, uintptr_t address, SIZE_T size, DWORD newProtect) {
    DWORD oldProtect;
    return VirtualProtectEx(hProcess, reinterpret_cast<void*>(address), size, newProtect, &oldProtect) != 0;
}
